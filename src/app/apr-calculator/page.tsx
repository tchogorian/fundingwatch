        ...
        <div className="rounded-2xl border p-4 sm:p-6 md:p-8" style={cardStyle}>
          {activeTab === 1 && (
            <>
              ...
              {tab1Result && tab1Positions.length > 0 && (
                <div className="mt-10 border-t pt-8" style={{ borderColor: "var(--color-border-default)" }}>
                  <h3 className="text-lg font-semibold" style={textPrimary}>
                    Add More Positions — Stacking Calculator
                  </h3>
                  {tab1Positions.map((pos, idx) => (
                    <div key={idx} className="mt-4 rounded-xl border p-4 flex flex-col gap-4" style={{ borderColor: "var(--color-border-default)" }}>
                      <span className="text-sm font-medium" style={textTertiary}>Position {idx + 1}</span>
                      <div className="grid gap-4 sm:grid-cols-2">
                        <div>
                          <label className="text-xs font-medium" style={textSecondary}>Advance ($)</label>
                          <input
                            type="number"
                            min={0}
                            value={pos.advance}
                            onChange={(e) => updateTab1(idx, { advance: e.target.value })}
                            className={inputCls}
                            style={textPrimary}
                          />
                        </div>
                        <div>
                          <label className="text-xs font-medium" style={textSecondary}>
                            {pos.useFactorRate ? "Factor rate" : "Total payback ($)"}
                          </label>
                          {pos.useFactorRate ? (
                            <input
                              type="number"
                              min={1.01}
                              step={0.01}
                              value={pos.factorRate}
                              onChange={(e) => updateTab1(idx, { factorRate: e.target.value })}
                              className={inputCls}
                              style={textPrimary}
                            />
                          ) : (
                            <input
                              type="number"
                              min={0}
                              value={pos.totalPayback}
                              onChange={(e) => updateTab1(idx, { totalPayback: e.target.value })}
                              className={inputCls}
                              style={textPrimary}
                            />
                          )}
                        ...
                        <div>
                          <label className="text-xs font-medium" style={textSecondary}>Term</label>
                          <select
                            value={pos.termIndex}
                            onChange={(e) => updateTab1(idx, { termIndex: Number(e.target.value) })}
                            className={inputCls}
                            style={textPrimary}
                          >
                            {TERM_OPTIONS_TAB1.map((opt, i) => (
                        ...
                        <div>
                          <label className="text-xs font-medium" style={textSecondary}>Origination fee ($)</label>
                          <input
                            type="number"
                            min={0}
                            value={pos.originationFee}
                            onChange={(e) => updateTab1(idx, { originationFee: e.target.value })}
                            className={inputCls}
                            style={textPrimary}
                          />
                        </div>
                      </div>
                      {tab1Positions.length > 1 && (
                        <button
                          type="button"
                          onClick={() => removeTab1(idx)}
                  ...
                  {tab1Positions.length < 5 && (
                    <button
                      type="button"
                      onClick={addTab1Position}
                      className="mt-4 w-full min-h-[48px] rounded-lg border border-dashed py-3 text-sm font-medium sm:w-auto sm:px-6"
